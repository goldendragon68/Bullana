...
router.post('/getGameResult', auth.authenticateUser, function(req, res) {
	...
	common.findUserBalance(userId, currency, function(balance){
		if(balance >= bet_amount) {
			userHash.findOne({user_id:userId,game:"dice"}).exec(async function(err, resp) {
				if(err) {
					return res.json({success:0,msg:"Please try again"});
				}	
				
				if(resp) {
					var result = game.diceGameResult(resp.server_seed, resp.client_seed, resp.nonce);
					var nounce= parseInt(resp.nonce);
					var serverSeed = resp.server_seed;
				    var clientSeed  = resp.client_seed;
				} else {
					var user_has = await game.generateUserHash(userId, "dice");
					var result = game.diceGameResult(user_has.client_seed, user_has.server_seed, user_has.nonce);
					var serverSeed = user_has.server_seed;
					var clientSeed = user_has.client_seed;
					var nounce 	   = parseInt(user_has.nonce); 
				}

				userHash.updateOne({user_id:userId,game:"dice"}, {"$set": {nonce: nounce+1}}).exec(function(err1, res1){
...
router.post('/updateUserHash', common.userVerify, function(req, res) {
	...
	userHash.updateOne({user_id:userId, game:"dice"}, {"$set": {client_seed:client_seed,server_seed:server_seed,nonce:nonce,new_client_seed:new_client_seed,new_server_seed:new_server_seed}}).exec(function(err, resp){
...
router.post('/getUserHash', common.userVerify, function(req, res){
	var userId = mongoose.mongo.ObjectId(req.userId);
	userHash.findOne({user_id:userId,game:"dice"}).exec(function(err, resp){
		if(err) {
			return res.json({success:0,msg:"Please try again."});
		}
...
router.post('/saveBetHistory', common.userVerify, async function(req, res){
	var currency = req.body.currency;
	var bet_amount = req.body.bet_amount;
	var bet_type = req.body.bet_type;
	var roll_value = req.body.roll_value;
	...
	common.findUserBalance(userId, currency, async function(userBalance){ 
		if(bet_amount <= userBalance) {
			if(bet_type == "roll_under") {
				if(result < roll_value) {
			...
			if(status == "loser"){
                common.findAdminBal(currency, async function(AdminBal){
	                ...
	                /*let profitJson = {user_id: mongoose.mongo.ObjectId(userId), currency: currency, amount: amount, type: "dice"}
			...
			betHistory.create(obj, function(err, resData) {
				...
				if(resData) {
					betHistory.find({user_id:mongoose.mongo.ObjectId(userId),game:"dice"}).sort({_id : -1}).exec(function(err1, res1){
						users.updateOne({"_id":userId,"gamecount.name":"dice"},{$inc:
    					{"gamecount.$.total_count":1}},(ers,doc)=>{
    						if(doc){
    							common.wagecountUpdate(currency,userId,bet_amount,amount,status,"dice",function(data) {});
								common.gamecountUpdate(userId,"dice",status,currency,function(data) {});
								common.sendGameResult(resData);
							 	res.json({success:1,status:obj.status,balance : updated_balance, currency: currency, status:obj.status , history : res1});
    						}else{
...
router.post('/getBetHistory', common.userVerify, function(req, res){
	var userId = req.userId;
	betHistory.find({user_id:mongoose.mongo.ObjectId(userId),game:"dice"}).sort({_id : -1}).limit(50).exec(function(err1, res1){
		res.json({success:1, history : res1});
	})
});

router.post('/getAllBetHistory', function (req, res) {
    betHistory.find({game:"dice"}).sort({_id : -1}).exec(function(err1, res1){
        if(res1.length !==0 ){
...
router.post('/getAllBetHis', function (req, res) {
    betHistory.find({game:"dice"}).sort({_id : -1}).limit(50).exec(function(err1, res1){
        if(res1.length !== 0 ){
            var i = 1; var len = res1.length;var storeData=[];
            res1.forEach((value) => {
                let id=value.user_id
...
router.post('/changeHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"dice"}).exec(function(err, resp) {
        ...
        if(resp){
        	var check=common.updateUserseed(userId,oldserverSeed,"dice");
            userHash.updateOne({user_id:userId,game:"dice"},{"$set":{client_seed:clientSeed, server_seed:serverSeed, nonce:nounce, new_server_seed:newSer, new_client_seed:newCli}}).exec(function(errs,resData) {
                if(errs){
...
router.post('/generateHash',common.userVerify, function (req, res) {
    ...
    userHash.findOne({user_id:userId,game:"dice"}).exec(function(err, resp) {
        ...
        if(resp){
            userHash.updateOne({user_id:userId,game:"dice"},{"$set":{new_client_seed:newCli}}).exec(function(errs,resData) {
                if(errs){
			...
			// if(resp.user_favourites.length > 0){
				...
				if(tot =="added"){
					users.updateOne({'_id':userId},{'user_favourites':storeData}).exec(function(errs,resData){
						...
						if(resData){
							gamelist.updateOne({'game_name':'dice'},{$inc:{'fav_count':1}}).exec(function(Err,Res){
								if(Err){
									return res.json({success:0,msg:"Something went wrong"});	
								}
								if(Res){
									return res.json({success:1,msg:"Added to Favourites",game:"dice"});	
								}
							})
						}
					})
			...
			// if(resp.liked_game.length > 0){
				...
				if(tot =="added"){
					users.updateOne({'_id':userId},{'liked_game':storeData}).exec(function(errs,resData){
						...
						if(resData){
							gamelist.updateOne({'game_name':'dice'},{$inc:{'like_count':1}}).exec(function(Err,Res){
								if(Err){
									return res.json({success:0,msg:"Something went wrong"});	
								}
								if(Res){
									return res.json({success:1,msg:"Added to Liked",game:"dice"});	
								}
							})
						}
					})
...
router.get('/getGlobal', function(req, res) {
	return res.json({success:1, msg:global.myGlobalVariable});
})

router.post('/getminbetamount', common.userVerify, function(req, res) {
	var userCurr = req.body.currency;
	/*gamelist.findOne({game_name:'dice'}).exec(function(err1, res1){
        if(res1){
           	res.json({success:1,min_bet:res1.min_bet, max_bet:res1.max_bet});
        } else {res.json({success:0});}
	})*/
	gamelist.findOne({game_name:'dice'}, {Curr:{$elemMatch:{currency:userCurr}}}).exec(function(err,resData){
		if(resData){
